...
<body>
	<%@ page
		import="com.lxisoft.contacts.model.Contact, java.util.*,java.io.*"%>
		...
		<div class="row">
			...
			<form id="form1" class="form-horizontal form_style col-sm-8"
				...
				<input class="col-sm-5" type="text" name="first_name"
					pattern="[^,]+" required="" value=<%=contact.getFirstName()%>>
				<br> <label class="col-sm-3"><%=props.getProperty("last_name")%></label>
				<input class="col-sm-5" type="text" name="last_name" pattern="[^,]+"
					...
					value=<%=contact.getPhoneNumber()%> placeholder="+91" readonly="">
				<br> <label class="col-sm-3"><%=props.getProperty("email")%>&nbsp;</label>
				<input class="col-sm-5" type="email" name="email" checked="checked"
					pattern="[^,]+" value=<%=contact.getEmail()%> required=""><br>
				<br> <input type="submit"
					value=<%=props.getProperty("update")%>>
			</form>
		</div>
	</div>
</body>
